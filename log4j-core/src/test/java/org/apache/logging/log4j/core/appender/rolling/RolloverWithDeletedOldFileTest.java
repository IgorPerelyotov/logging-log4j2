...
public class RolloverWithDeletedOldFileTest {
  ...
  public void testAppender() throws Exception {
    ...
    for (final File file : files) {
      ...
      if (file.getName().equals("test-005.log")) {
        fileToRemove = file;
      }
    }
    fileToRemove.delete();
    long timeBeforeNewRollover = System.currentTimeMillis();
    for (int i = 0; i < 10; ++i) {
      // 30 chars per message: each message triggers a rollover
      logger.fatal("This is a test message number " + i); // 30 chars:
    }
    ...
    for (final File file : files) {
      ...
      if (file.lastModified() < timeBeforeNewRollover) {
        fail("file was not changed " + file);
      }
    }
  }
}
